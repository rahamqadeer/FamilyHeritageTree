...
// Invite member via email with access level (view | edit)
router.post('/:familyId/invite', requireFamilyRole(['ADMIN']), async (req, res) => {
  try {
    const { email, role, accessLevel = 'edit' } = req.body
    ...
    const inviterName =
      req.auth.user.display_name || req.auth.supabaseUser?.email || 'A family admin'

    const { invite, inviteLink, emailResult } = await createFamilyInvitation({
      familyId,
      email,
      accessLevel,
      role: accessLevel === 'view' ? 'JUNIOR' : (role || 'ADULT'),
